...
const { addProductImage } = require('../controllers/product');
...
function addFileToProduct(req, res, next) {
  ...
  const fileDirectoryLength = fs.readdirSync(`uploads/${filePath}`).length;
  ...
  // save into the product the information about his uploaded image
}

// Each time an image is uploaded, it's stored in a directory with the name equal
// to the id of the product

// single image (featured image) upload for product
router.post('/product/:id/image', addFileToProduct);

router.delete('/product/:id/image/:name', (req, res, next) => {
  try {
...
router.delete('/product/:id/video/:name', (req, res, next) => {
  try {
...
router.post('/product/:id/images', (req, res, next) => {
  // req.file is the name of your file in the form above
  // req.body will hold the text fields, if there were any
  ...
  return addProductAdditionalImage(req, res, next);
  // save into the product the information about his uploaded image
});

// Delete one additoonal images of a product
router.delete('/product/:id/images/:name', (req, res, next) => {
  try {
    fs.unlinkSync(`uploads/${req.params.id}/${req.params.name}`);
  } catch (err) {
    return next(err);
